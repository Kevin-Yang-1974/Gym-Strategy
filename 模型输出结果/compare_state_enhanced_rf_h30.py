# -*- coding: utf-8 -*-
"""比较状态增强随机森林、普通随机森林、Logistic-GAM近似和 Cox 的 H=30续费概率预测效果。"""
...
REPORT_PATH = OUT_DIR / "5.3_状态增强随机森林与对比模型结果_H30.md"
METRIC_PATH = OUT_DIR / "5.3_状态增强随机森林与对比模型指标_H30.csv"
IMPORTANCE_PATH = OUT_DIR / "5.3_状态增强随机森林特征重要性_H30.csv"
PRED_PATH = OUT_DIR / "5.3_状态增强随机森林与对比模型预测_H30.csv"
...
class CoxBreslowModel:
	...
	def predict_prob(self, x_train, train_duration, train_event, x_eval):
		baseline_s30 = self.baseline_survival_at(x_train, train_duration, train_event,30.0)
		risk_score = x_eval @ self.beta
		pred =1.0 - np.power(baseline_s30, np.exp(np.clip(risk_score, -50,50)))
...
def save_metric_plot(metric_df):
	...
	ax.legend()
	fig.tight_layout()
	fig.savefig(OUT_DIR / "5.3_图13_状态增强随机森林与对比模型指标_H30.png", bbox_inches="tight")
	plt.close(fig)
...
def save_probability_plot(pred_df):
	...
	ax.legend()
	fig.tight_layout()
	fig.savefig(OUT_DIR / "5.3_图14_状态增强随机森林与对比模型概率分布_H30.png", bbox_inches="tight")
	plt.close(fig)
...
def save_importance_plot(importance):
	...
	fig.tight_layout()
	fig.savefig(OUT_DIR / "5.3_图15_状态增强随机森林特征重要性Top15_H30.png", bbox_inches="tight")
	plt.close(fig)
...
def write_report(metric_df, importance, train_df, test_df, cutoff):
	...
	report = f"""#5.3状态增强随机森林与对比模型结果 H=30
...
二分类模型使用 `censored=0` 的完整观察样本。为了与 Cox比较，Cox模型使用训练期全部样本估计生存风险，并在相同测试集上输出 $H=30$续费概率。样本按 `end_date` 做时间切分，切分日期为 `{cutoff.date()}`。
